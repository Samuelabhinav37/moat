...
**Status: done (v0.11.13), reverted (v0.11.16), redone differently (v0.11.40).** The v0.11.13
version put a click-to-expand drill-down inside the popup itself and was reverted as unwanted popup
UI. v0.11.40 keeps the popup's flat "By company" list untouched and moves the detail to a new
**Settings → Trackers** tab: the same per-tab company list, each with a one-sentence description
and website link from TrackerDB (`rules/dnr/company-info.json`, built at filter-update time).
